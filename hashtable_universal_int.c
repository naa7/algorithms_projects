...
typedef struct l_node {  key_t      key;
                         object_t  *obj;
                   struct l_node  *next; } list_node_t;

typedef struct { int a; int b; int size; } hf_param_t;

typedef struct { int         size;
                 list_node_t **table;                 
                 int (*hash_function)(key_t, hf_param_t); 
                 hf_param_t hf_param; }    hashtable_t;
...
int universalhashfunction(key_t key, hf_param_t hfp)
...
void insert(hashtable_t *ht, key_t new_key, object_t *new_obj)
{  int i; list_node_t *tmp_node;
...
{  int i; list_node_t *tmp_node; object_t *tmp_obj;
...
object_t *find_mtf(hashtable_t *ht, key_t query_key)
{  int i; list_node_t *front_node, *tmp_node1, *tmp_node2;
   i = ht->hash_function(query_key, ht->hf_param );
   front_node = tmp_node1 = (ht->table)[i]; tmp_node2 = NULL;
